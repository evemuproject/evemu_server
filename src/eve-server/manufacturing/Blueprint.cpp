...
uint32 Blueprint::_Spawn(
    ...
    const BlueprintType *bt = sItemFactory.GetBlueprintType(data.typeID);
    if(bt == NULL)
        return 0;
    ...
    if(!sItemFactory.db().NewBlueprint(blueprintID, bpData)) {
        // delete item
...
void Blueprint::Delete() {
    ...
    sItemFactory.db().DeleteBlueprint(m_itemID);
    // redirect to parent
