...
PyResult BeyonceBound::Handle_CmdGotoBookmark(PyCallArgs &call) {
    ...
    BookmarkService *bkSrvc = (BookmarkService *)(sManager.LookupService( "bookmark" ));

    if( bkSrvc == NULL )
    {
        sLog.Error( "BeyonceService::Handle_GotoBookmark()", "Attempt to access BookmarkService via (BookmarkService *)(sManager.LookupService(\"bookmark\")) returned NULL pointer." );
        return NULL;
    }
    else
    {
...
PyResult BeyonceBound::Handle_CmdWarpToStuff(PyCallArgs &call) {
    ...
    else if( arg.type == "bookmark" )
    {
        ...
        if( bkSrvc == NULL )
        {
            sLog.Error( "BeyonceService::Handle_WarpToStuff()", "Attempt to access BookmarkService via (BookmarkService *)(sManager.LookupService(\"bookmark\")) returned NULL pointer." );
            return NULL;
        }
        else
        {
