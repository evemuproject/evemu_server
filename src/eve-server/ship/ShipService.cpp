...
PyResult ShipBound::Handle_Board(PyCallArgs &call) {
    ...
    ShipRef boardShipRef = sItemFactory.GetShip( args.arg1 );                 // This is too inefficient, use line below
    ...
    else
    {
        if( boardShipRef->ValidateBoardShip( boardShipRef, call.client->GetChar()) )
        {
            ...
            ////sItemFactory.GetShip( call.client->GetShipID() )->Relocate( GPoint(0.0,0.0,0.0) );
...
PyResult ShipBound::Handle_AssembleShip(PyCallArgs &call) {
    ...
    ShipRef ship = sItemFactory.GetShip( itemID );
...
PyResult ShipBound::Handle_Drop(PyCallArgs &call) {
    ...
    for( uint32 i=0; i<PyToDropList->size(); i++ )
    {
        ...
        cargoItem = sItemFactory.GetItem( itemID );
        if( !cargoItem )
        {
        ...
        groupID = sItemFactory.GetItem( itemID )->groupID();
        categoryID = sItemFactory.GetItem( itemID )->categoryID();
            ...
            || (groupID == EVEDB::invGroups::Freight_Container) )
        {
            ...
            cargoContainerItem = sItemFactory.GetCargoContainer( itemID );
        ...
        else if( categoryID == EVEDB::invCategories::Structure )
        {
            ...
            structureItem = sItemFactory.GetStructure( itemID );
            Client * who = call.client;
            GPoint location( who->GetPosition() );
            radius = 1500.0;
            theta = MakeRandomFloat( 0.0, (2*M_PI) );
...
PyResult ShipBound::Handle_Jettison(PyCallArgs &call) {
    ...
    for(; cur != end; cur++)
    {
        ...
        groupID = sItemFactory.GetItem( *cur )->groupID();
        categoryID = sItemFactory.GetItem( *cur )->categoryID();
            ...
            || (groupID == EVEDB::invGroups::Freight_Container) )
        {
            ...
            cargoContainerItem = sItemFactory.GetCargoContainer( *cur );
            if( !cargoContainerItem )
                throw PyException( MakeCustomError( "Unable to spawn item of type %u.", cargoContainerItem->typeID() ) );
        ...
        else if( categoryID == EVEDB::invCategories::Structure )
        {
            ...
            structureItem = sItemFactory.GetStructure( *cur );
            if( !structureItem )
                throw PyException( MakeCustomError( "Unable to spawn Structure item of type %u.", structureItem->typeID() ) );
        ...
        else if( categoryID == EVEDB::invCategories::Deployable )
        {
            ...
            cargoItem = sItemFactory.GetItem( *cur );
            if( !cargoItem )
                throw PyException( MakeCustomError( "Unable to spawn Deployable item of type %u.", cargoItem->typeID() ) );
        ...
        else
        {
            ...
            if( !cargoItem )
            {
                ...
                newJettisonCargoContainerItem = sItemFactory.SpawnCargoContainer( *p_idata );
...
PyResult ShipBound::Handle_Eject(PyCallArgs &call) {
    ...
    ShipRef capsuleRef = sItemFactory.SpawnShip( idata );
    if( !capsuleRef )
    ...
    ShipRef oldShipRef = sItemFactory.GetShip( oldShipItemID );
...
PyResult ShipBound::Handle_ActivateShip(PyCallArgs &call)
{
	...
	ShipRef oldShipRef = call.client->GetShip();
    ShipRef newShipRef = sItemFactory.GetShip(newShip);
