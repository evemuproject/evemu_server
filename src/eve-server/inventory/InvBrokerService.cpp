...
PyResult InvBrokerBound::Handle_GetInventoryFromId(PyCallArgs &call) {
    ...
    Inventory *inventory = sItemFactory.GetInventory( args.arg1 );
    if(inventory == NULL) {
    ...
    PyRep *result = sManager.BindObject(call.client, ib);
...
PyResult InvBrokerBound::Handle_GetInventory(PyCallArgs &call) {
    ...
    sItemFactory.SetUsingClient( call.client );
    Inventory *inventory = sItemFactory.GetInventory( m_entityID );
    if(inventory == NULL) {
