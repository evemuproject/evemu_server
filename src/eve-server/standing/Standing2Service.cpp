...
PyResult Standing2Service::Handle_GetCharStandings(PyCallArgs &call) {
    ...
    return(sManager.cache_service->MakeObjectCachedSessionMethodCallResult(method_id, "charID"));
}
PyResult Standing2Service::Handle_GetCorpStandings(PyCallArgs &call) {
    ...
    return(sManager.cache_service->MakeObjectCachedSessionMethodCallResult(method_id, "corpID"));
}
